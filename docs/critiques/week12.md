...
- **Two-notebook split lets us compare configurations cleanly.** Keeping the head-only mock run (`week12_wordle_es_gemma.ipynb`) separate from the full-vocab LoRA sweep (`week12_wordle_es_lora_run.ipynb`) makes the contrast between the working and failing setups easy to point at.
...
### What the PASS-A verdict means

The Section 3 grading rubric is a three-way decision tree designed so each outcome isolates a different failure mode. Reading the Session 8 numbers through it:

- **Criterion 1 — `best_greedy − post_ws_greedy ≥ +10pp` → True (+22pp).** This is the "did ES ever find a better iterate than warm-start did?" test. It fires iff ES is actually producing signal on top of the supervised warm-start baseline. Session 7 passed this too (+18pp at iter 1), so on its own this criterion only rules out the degenerate "ES is just drifting around post-WS" reading — it does *not* distinguish between a converged run and a lucky early iterate.
- **Criterion 2 — `final_greedy ≥ post_ws_greedy` → True (+16pp).** This is the "did ES still hold the gain at the end of the run, or did it walk away from it?" test. **This is the criterion Session 7 failed** (`final = 18%`, `post_ws = 30%`, so `final − post_ws = −12pp`). Session 8 passing it is the single biggest change in the Week 12 picture: aggregation is no longer the bottleneck, and the per-iter updates compose instead of fighting each other.
- **Criterion 3 — `dprobe` non-zero on ≥ 25% of iters → True (58%).** This is the plumbing-integrity check: `dprobe` ≠ 0 means the ES gradient actually moves the policy's action distribution on the held-out probe. Failure here would mean the subset-CRN wiring is broken (all perturbations cancel, or the subset is degenerate); passing at 58% means the ES gradient estimate is non-degenerate on a solid majority of iters.
- **PASS-B discriminator — `best_greedy − final_greedy ≥ 10pp` → False (only +6pp).** This is what PASS-A actually *adds* over PASS-B. PASS-B fires when ES finds the gain but then collapses away from it by ≥10pp, which would mean best-iter restore is doing the work and ES itself is still too unstable to converge. The Session 8 `best − final = +6pp` gap is **inside the ~7pp single-slate eval noise floor** (50 episodes × 16 secrets gives σ ≈ 3-4pp per point under the per-iter RNG stream), so there is no evidence of collapse — `best_greedy = 58% @ iter 1` vs `final_greedy = 52% @ iter 59` is consistent with ES sitting in a stable basin whose greedy eval bounces within a ±4pp window of ~50%. This is what it means for PASS-A to fire and PASS-B not to: the step-size fix is *self-sufficient*, not merely compensable.

**What the verdict rules in (strong claims).** The step-size / overshoot hypothesis from Session 7 is confirmed end-to-end. With rotating 4-word mini-batch objectives, the iter-0 ALPHA calibration overshoots by roughly `1 / ALPHA_SCALE = 4×`: quartering the step lets 60 iters of accumulation land near the 0.13 per-iter calibration target (empirically `‖θ−θ₀‖` grew 0.66 → ~4.9, linear, non-oscillating) instead of producing the Session 7 random-walk trajectory. Mini-batch ES under CRN is now a working configuration at vocab = 16 — not just signal-productive (Session 7) but end-to-end-useful.

**What the verdict does not prove (honest caveats).**

1. **Eval noise, not optimizer skill, upper-bounds what we can measure.** The per-iter greedy trajectory still oscillates 30-58% even while `‖θ−θ₀‖` is monotonic — that is a measurement artifact of the 50-episode probe slate, not ES instability. Any claim finer than "ES is converging within a ±4pp basin" is below the measurement floor; we cannot, for example, distinguish "iter 59 is the actual optimum" from "iter 1 was, and everything since is a random walk within the basin." The post-run eval on restored-best θ printing 38% (vs `best_greedy = 58%`) is the same phenomenon — a different `probe_seed` draws a different slate, and the 20pp gap is ~2.5σ of single-slate noise.
2. **PASS-A is single-seed and single-stage.** One seed, one stage (`VOCAB_SCHEDULE=[16]`), no multi-seed replication. The hypothesis is confirmed directionally but the effect size (+22pp best, +16pp final) could move under reseeding; nothing above `+10pp` should be treated as calibrated.
3. **It does not generalize to larger vocab yet.** The full-curriculum regime (`VOCAB_SCHEDULE=[8, 16, 32, …, 1024]`) is untouched by this session. The PASS-A result means "the Session 7 aggregation bug is fixed on a single 16-word probe," not "ES + warm-start + LoRA scales to full Wordle." Those are two very different claims.
4. **Best-iter restore is validated but not required here.** PASS-A fired without PASS-B triggering, which means the restore-best machinery wasn't load-bearing in this run. It remains a useful robustness feature for configurations where ALPHA isn't cleanly tuned, and the plumbing now exists in both `train_es_wordle` and `train_curriculum` — but this specific run does not provide evidence for or against its necessity at larger vocabs.

**Operational implication.** The measurement floor is now the binding constraint, not the optimizer. Next cheapest experiment (single-seed): replace the 50-episode probe slate with the deterministic 16 × K-episodes-per-secret full-pool eval so the per-iter noise drops from ~7pp to <3pp. Only after that produces a visually monotonic trajectory does it make sense to either (a) reseed for replication, or (b) scale to `VOCAB_SCHEDULE=[32]` and check whether PASS-A still fires. Reversing that order would conflate "does the fix scale?" with "can we measure whether it scales?"
