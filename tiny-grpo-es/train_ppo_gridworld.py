"""
PPO (Proximal Policy Optimization) baseline for GridWorld.
Standard action-space RL method for comparison with ES.
"""
import numpy as np
import torch
...
def evaluate_policy(
    ...
    """Evaluate policy (same as ES version)."""
    total_rewards = []
    successes = []
    episode_steps = []
        ...
        if (iteration + 1) % eval_every == 0 or iteration == 0:
            ...
            if log_wandb and WANDB_AVAILABLE:
                wandb.log({
                    ...
                    "train_reward": mean_episode_reward,
...
def main():
    ...
    # Networks
    policy = PolicyNetwork(state_dim, action_dim, hidden_dim=64, n_layers=2).to(device)
    value_net = ValueNetwork(state_dim, hidden_dim=64, n_layers=2).to(device)
    
    # Initialize wandb
    if WANDB_AVAILABLE:
        wandb.init(
            project="stat-4830-es-gridworld",
            name=f"PPO-grid{env_kwargs['size']}-harder{use_harder_env}",
            config={
                "seed": seed,
                ...
                "value_params": value_net.count_parameters(),
                "method": "PPO",
            }
        )
    ...
    torch.save(policy.state_dict(), output_dir / f"ppo_policy_{env_class.__name__}.pt")
    torch.save(value_net.state_dict(), output_dir / f"ppo_value_{env_class.__name__}.pt")
